...
def _label_point(geom) -> tuple[float, float]:
    ...
    return round(point.y, COORD_DECIMALS), round(point.x, COORD_DECIMALS)
...
def build_zones_geojson() -> dict:
    ...
    for bidding_zone, entsoe_file in BIDDING_ZONE_TO_ENTSOE_FILE.items():
        ...
        label_lat, label_lon = _label_point(clipped)
        geometry = _simplify_and_round(clipped, ZONE_SIMPLIFY_TOLERANCE)
        properties = {"bidding_zone": bidding_zone, "label_lat": label_lat, "label_lon": label_lon}
    ...
    for feature in ne_countries["features"]:
        ...
        if iso_a2 in iso_to_bidding_zone:
            ...
            label_lat, label_lon = _label_point(geom)
            geometry = _simplify_and_round(geom, ZONE_SIMPLIFY_TOLERANCE)
            features.append(
                {
                    "type": "Feature",
                    "properties": {
                        "bidding_zone": iso_to_bidding_zone[iso_a2],
                        "label_lat": label_lat,
                        "label_lon": label_lon,
                    },
                    "geometry": geometry,
